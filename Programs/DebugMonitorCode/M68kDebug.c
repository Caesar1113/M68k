...
void Load_SRecordFile()
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < NumDataBytesToRead; i++)
        { // read in remaining data bytes (ignore address and checksum at the end
...
void MemoryChange(void)
{
    ...
    while (1)
    {
        ...
        { // are we trying to change data at this location by entering a hex char
...
************************************************************************************/
// Can use as Write and Read
// When Read, make c a dummy variable
int WriteSPIChar(int c)
{
    ...
    // SPI_data works as write buffer now
    SPI_Data = c;
    WaitForSPITransmitComplete();

    // SPI_data works as read buffer now
    data = SPI_Data;
...
{
    return (unsigned char)WriteSPIChar(0);
}
...
{
    Enable_SPI_CS();
    // WriteSPIInstruction(0x05, NULL);
    while ((char)SPI_Status << 7 == 0b10000000){
        // Keep running until Busy Bit set to 0
    }
    Disable_SPI_CS();
}
...
void WriteSPIFlashData( int FlashAddress, unsigned char *MemoryAddress, int size)
{
    ...
    // Enable flash bit as we are writting to flash
    Enable_SPI_CS();

    // Need to enable write
    WriteSPIInstruction(0x06, NULL);
    WriteSPIInstruction(0x02, FlashAddress);

    for (i = 0; i < size; i++)
    {
        WriteSPIChar(MemoryAddress[i]);
    }

    // Disable flash bit, considered as the end of command
    Disable_SPI_CS();
    WaitSPIBusy();
}
...
void ReadSPIFlashData( int FlashAddress, unsigned char *MemoryAddress, int size)
{
    ...
    // Enable flash bit as we are writting to flash (read)
    Enable_SPI_CS();
    WriteSPIInstruction(0x03, FlashAddress);
...
{
    Enable_SPI_CS();
    // enable writing before any new write or erase command
    WriteSPIInstruction(0x06, NULL);
    WriteSPIInstruction(0xC7, NULL);
...
void ProgramFlashChip(void)
{
    ...
    unsigned char *programPointer = 0x08000000;
    ...
    // It will be necessary to erase the chip (or the appropriate blocks or sectors) before programming it
    EraseSPI();
    SPI_Init();

    printf("\r\nProgramming SPI Flash....");

    // Need to write first 256k = 1024 * 256 to flash chip
    for (i = 0; i < 1024; i++)
    {
        for (j = 0; j < 256; j++)
        {
            // Copy to write buffer
            writeBuffer[j] = programPointer[i * 256 + j];

            // Print progress
            if (j == 0 && i % 32 == 0)
            {
                printf("\r\nValue 0x%x was read at addr 0x%x, will be wrote to flash.\n", programPointer[i * 256 + j], &programPointer[i * 256 + j]);
            }
        }
        WriteSPIFlashData(flashAddr, writeBuffer, 256);
    ...
    {
        // Get read value
        ReadSPIFlashData(flashAddr, readBuffer, 256);
        for (j = 0; j < 256; j++)
        {
            // Checking one by one
            if (readBuffer[j] != programPointer[i * 256 + j])
            {
                pass = 0;
                printf("\r\nErr: Value 0x%x was read at addr 0x%x, should be 0x%x\n", readBuffer[j], &readBuffer[j], programPointer[i * 256 + j]);
                break;
            }
        }
    ...
    if (pass == 1)
    {
        printf("\r\nProgramming Finished And Is Verified!");
    }
    else
    {
        printf("\r\nProgramming Failed!");
...
void LoadFromFlashChip(void)
{
    ...
    unsigned char *programPointer = 0x08000000;
    unsigned char readBuffer[256];
    int flashAddr = 0; // copy the first 256k bytes
    ...
    for (i = 0; i < 1024; i++)
    {
        ...
        {
            // Checking one by one
            programPointer[i * 256 + j] = readBuffer[j];

            // Print progress
            if (j == 0 && i % 32 == 0)
            {
                printf("\r\nValue 0x%x was write to addr 0x%x.\n", readBuffer[j], &programPointer[i * 256 + j]);
            }
        }
    ...
    printf("\r\nLoading Program From SPI Flash Finished!");
}
...
void DumpRegisters()
{
    ...
    printf("   [");
    if ((SR & (unsigned short int)(0x8000)) == (unsigned short int)(0x8000))
        putchar('T');
    else
        putchar('-'); // Trace bit(bit 15)
    if ((SR & (unsigned short int)(0x2000)) == (unsigned short int)(0x2000))
        putchar('S');
    else
        putchar('U'); // supervisor bit  (bit 13)

    if ((SR & (unsigned short int)(0x0400)) == (unsigned short int)(0x0400))
        putchar('1');
    else
        putchar('0'); // IRQ2 Bit (bit 10)
    if ((SR & (unsigned short int)(0x0200)) == (unsigned short int)(0x0200))
        putchar('1');
    else
        putchar('0'); // IRQ1 Bit (bit 9)
    if ((SR & (unsigned short int)(0x0100)) == (unsigned short int)(0x0100))
        putchar('1');
    else
        putchar('0'); // IRQ0 Bit (bit 8)

    if ((SR & (unsigned short int)(0x0010)) == (unsigned short int)(0x0010))
        putchar('X');
    else
        putchar('-'); // X Bit (bit 4)
    if ((SR & (unsigned short int)(0x0008)) == (unsigned short int)(0x0008))
        putchar('N');
    else
        putchar('-'); // N Bit (bit 3)
    if ((SR & (unsigned short int)(0x0004)) == (unsigned short int)(0x0004))
        putchar('Z');
    else
        putchar('-'); // Z Bit (bit 2)
    if ((SR & (unsigned short int)(0x0002)) == (unsigned short int)(0x0002))
        putchar('V');
    else
        putchar('-'); // V Bit (bit 1)
    if ((SR & (unsigned short int)(0x0001)) == (unsigned short int)(0x0001))
        putchar('C');
    else
        putchar('-'); // C Bit (bit 0)
    putchar(']');
...
void ChangeRegisters(void)
{
    ...
    else if (reg[0] == (char)('a'))
    { // change address register, a7 is the user stack pointer, sp is the system stack pointer
...
void SetBreakPoint(void)
{
    ...
    if ((BPAddress & 0x00000001) == 0x00000001)
    { // cannot set BP at an odd address
        printf("\r\nError : Break Points CANNOT be set at ODD addresses");
        return;
    }

    if (BPAddress < 0x00008000)
    { // cannot set BP in ROM
...
void SetWatchPoint(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if (WatchPointAddress[i] == WPAddress && WPAddress != 0)
        { // so we can set a wp at 0
...
void HandleBreakPoint(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if (BreakPointAddress[i] == PC)
        { // if we have found the breakpoint
            BreakPointAddress[i] = 0;
            BreakPointSetOrCleared[i] = 0;
            *ProgramBreakPointAddress = BreakPointInstruction[i]; // put original instruction back
            BreakPointInstruction[i] = 0;
...
void menu(void)
{
    ...
    while (1)
    {
        ...
        { // Next instruction command
            DisableBreakPoints();
            if (Trace == 1 && GoFlag == 1)
            {                       // if the program is running and trace mode on then 'N' is valid
        ...
        { // if user choses to end trace and run program
            Trace = 0;
            TraceException = 0;
...
void MemoryTest(void)
{
    ...
    char c = 0;
    unsigned long test_data; // used to distinguish option of carrying out a test using bytes, words or long words
    int pattern_err = 0;
    int type_err = 0;
    int read_err = 0;
    int addr_err = 0;
    ...
    unsigned char *RamPtrByte;
    unsigned short *RamPtrWord;
    unsigned long *RamPtrLong;

    int pass = 0;

    /*
     * This block is used to determine Pattern
     */
    ...
    {
        pattern_err = 0;
        printf("\r\n type numbers to indicate test pattern, eg: 1, end with "
               "Enter"
               "");
        ...
        {
            pattern_err = 1;
            printf("Err: invalid pattern type\n");
        }
    } while (pattern_err == 1);

    /*
     * This block is used to determine Type
     */
    printf("\r\n test data pattern is choosen to be 0X%x\n", test_data);

    do
    {
        type_err = 0;
        printf("\r\nEnter 1: 'Byte', 2: 'Word', or 3: 'Long' for bytes, words, or long word: (please enter the type end with 'Enter')");
        printf("\r\n#");
        ...
        if (test_type == 1) // 8bit -> char / uint8_t
        {
        ...
        {
            printf("Testing: Word\n");
            wval = test_data | test_data << 8; // creating 16 bits word
            bval = test_data;
            break;
        }
        else if (test_type == 3)
        {
            printf("Testing: Long Word\n");
            lwval = test_data | test_data << 8 | test_data << 16 | test_data << 24; // creating 32 bits long word
            bval = test_data;
            break;
        }
        else
        {
            type_err = 1;
            printf("Err: invalid test type\n");
        }
    } while (type_err == 1);

    /*
     * This block is used to determine Address Range
     * End address is inclusive
     * When use word / long address can not be odd
     */
    printf("\r\nSetting up test address range: 0x09000000 to 0x097FFFFF\n");
    do
    {
        addr_err = 0;
        printf("\r\nStart Address: ");
        ...
        if (Start < 0x09000000)
        {
            printf("Err: start address smaller than 0x09000000\n");
            addr_err = 1;
        }
        else if (Start > 0x097FFFFF)
        {
            printf("Err: start address larger than 0x097FFFFF\n");
            addr_err = 1;
        }
        else if ((test_type == 2 || test_type == 3) && Start % 2 != 0)
        {
            printf("Err: start address is not an even address when input type is %s\n", test_type);
            addr_err = 1;
        }
    } while (addr_err == 1);
    ...
    do
    {
        ...
        if (End < 0x09000000)
        {
            printf("Err: end address smaller than 0x09000000\n");
            addr_err = 1;
        }
        else if (End > 0x097FFFFF)
        {
            printf("Err: end address larger than 0x097FFFFF\n");
            addr_err = 1;
        }
        else if (End < Start)
        {
            printf("Err: end address smaller than start address\n");
            addr_err = 1;
        }
        else if ((test_type == 2 || test_type == 3) && End % 2 != 0)
        {
            printf("Err: end address is not an even address when input type is %s\n", test_type);
            addr_err = 1;
        }
    } while (addr_err == 1);
    ...
    while (RamPtrByte <= End && RamPtrWord <= End && RamPtrLong <= End)
    {
        if (test_type == 1)
        {
            *RamPtrByte = bval;
            if (pass <= 0)
            {
                printf("\r\nWrite: 0x%x to addr 0x%x\n", *RamPtrByte, RamPtrByte);
                pass = 1000;
            }
            else
            {
        ...
        else if (test_type == 2)
        {
            if (RamPtrWord + 1 > End)
            {
                RamPtrByte = RamPtrWord;
                while (RamPtrByte <= End)
                {
                    *RamPtrByte = bval;
                    if (pass <= 0)
                    {
    ...
    if (RamPtrByte > End || RamPtrWord > End || RamPtrLong > End)
    {
    ...
    while (RamPtrByte <= End && RamPtrWord <= End && RamPtrLong <= End)
    {
        if (test_type == 1)
        {
            ...
            if (*RamPtrByte != bval)
            {
                printf("Err: Value 0x%x was read, value 0x%x was expected at address 0x%x\n", *RamPtrByte, bval, RamPtrByte);
                printf("Read Test failed, Memory Test failed\n");
                return;
            }
            RamPtrByte++;
        }
...
void DecodeBWLDataAfterOpCode(unsigned short int *OpCode)
{
    ...
    // bugger - have to build a special case and look at opcode to see what instruction is
        ...
        sprintf(TempString, "#$%X", (unsigned int)(OpCode[1]));
    else if ((OperandSize == (char)(2)) || (OperandSize == (char)(6)) || (OperandSize == (char)(7)))
    {                                                                                               // # long value
        sprintf(TempString, "#$%X", ((unsigned int)(OpCode[1]) << 16) | (unsigned int)(OpCode[2])); // create 3
        InstructionSize += 1;
    }
...
void Decode6BitEA(unsigned short int *OpCode, int EAChoice, unsigned short int DataSize, unsigned short int IsItMoveInstruction) // decode Mode/Register
{
    ...
    if (OperandMode == (unsigned char)(0))
    { // Effective Address = Dn
        RegNumber[0] = 'D';
    ...
    else if (OperandMode == (unsigned char)(1))
    { // Effective Address = An
        Decode3BitAddressRegister(OperandRegister);
    }

    else if (OperandMode == (unsigned char)(2))
    { // Effective Address = (An)
        strcatInstruction("(");
        Decode3BitAddressRegister(OperandRegister);
        strcatInstruction(")");
    ...
    else if (OperandMode == (unsigned char)(3))
    { // Effective Address = (An)+
        strcatInstruction("(");
        Decode3BitAddressRegister(OperandRegister);
        strcatInstruction(")+");
    ...
    else if (OperandMode == (unsigned char)(4))
    { // Effective Address = -(An)
        strcatInstruction("-(");
        Decode3BitAddressRegister(OperandRegister);
        strcatInstruction(")");
    ...
    else if (OperandMode == (unsigned char)(5))
    { // Effective Address = (d16, An)
        sprintf(TempString, "%d(A%d)", ExWord1, OperandRegister);
        strcatInstruction(TempString);
        InstructionSize += 1;
    }

    else if (OperandMode == (unsigned char)(6))
    { // Effective Address = (d8, An, Xn)
    ...
    {
        if (OperandRegister == 0)
        { // EA = (xxx).W
            sprintf(TempString, "$%X", ExWord1);
            strcatInstruction(TempString);
            InstructionSize += 1;
        }
        else if (OperandRegister == 1)
        {                                                                                          // EA = (xxx).L
        ...
        else if (OperandRegister == 4)
        {                                 // source EA = #Immediate addressing
...
void DisassembleInstruction(short int *OpCode) // pointer to Opcode
{
        ...
        ((*OpCode & (unsigned short int)(0xFEC0)) == (unsigned short int)(0xE4C0)))
    {
        ...
        else
        {
            ...
            if ((*OpCode & (unsigned short int)(0x0020)) == (unsigned short int)(0))
            { // if shift count defined by #value (bit 5 = 0), e.g. asl #3,d0
                sprintf(TempString, "#$%X,D%X",
                        ((*OpCode >> 9) & (unsigned short int)(0x0007)),
                        (*OpCode & (unsigned short int)(0x0007)));
            }
            else
            { // if shift is for example ASR D1,D2
    ...
    if ((*OpCode & (unsigned short int)(0xF000)) == (unsigned short int)(0x6000))
    {
        ...
        if (Displacement8Bit == (unsigned short int)(0))
    ...
    if ((*OpCode & (unsigned short int)(0xFF00)) == (unsigned short int)(0x4A00))
    {
        ...
        if ((*OpCode != (unsigned short int)(0x4AFC)) && (Size != (unsigned short int)(0x0003)))
        { // test for size to eliminate TAS instruction which shares similar opcode
            InstructionSize = 1;
